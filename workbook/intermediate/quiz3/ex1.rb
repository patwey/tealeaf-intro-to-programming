# The object for the integer 42 is reused when it is pointed to by more than one variable
# The object_ids change after reassignment (the variables point to new objects).
# Inside the block, the outer variables are changed to point to the objects created by reassignment
# After the block, the inner and outer variables point to the same objects with the same object_ids
def fun_with_ids
  a_outer = 42
  b_outer = "forty two"
  c_outer = [42]
